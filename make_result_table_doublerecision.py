...
file_name = "DoublePrecisionResult.txt"
...
with open(file_name) as file:
    for line in file:
        if line[0].isdigit():
            ...
            accuracy = np.round(float(accuracy), 4).astype(float)
            if row == col:
...
for key, table in result_tables.items():
    ...
    for row in range(9):
        for col in range(9):
            mean = np.round(np.mean(table[row][col])*100, 2).astype(float) 
            std = np.round(np.std(table[row][col])*100, 2).astype(float)
